...
- **Flash 布局：** P00 应用区 ~8KB（0x08000000）+ 固件缓存区 48KB（0x08004000）
...
- **CAN 转发 3 状态：** `APP_WAIT_UPDATE_CMD` -> `APP_WAIT_READY` -> `APP_UPDATE_SEND` -> 循环
- **READY 流控：** 目标设备擦除 W25Q16 完成后发 READY，网关收到才开始发 DATA，避免 FIFO 溢出
...
串口打印仅在错误和关键状态变化时使用。日志前缀：`[Host]` 上位机操作、`[DL]` Flash 下载。printf 字符串使用英文（ARM Compiler V5 限制）。
...
`HAL_Init -> SystemClock_Config -> MX_GPIO_Init -> MX_DMA_Init -> MX_CAN_Init -> MX_USART1_UART_Init -> CAN_Buf_Init -> AppUpdate_Init(Flash数据, 1344) -> UART_DMA_Rx_Init`
